...
which may be composed together:
  - [FileMod](#filemod): publish static files
  - [RouteMap](#routemod): route to a sub-module based on a URI path name
  - [PipelineMod](#pipelinemod): run through a serial list of sub-modules
  - [LogMod](#logmod): logs using W3C extended log format
...
# LogMod
[LogMod](webmod::LogMod) class is used to generate a server log file for all
HTTP requests in the [W3C Extended Log File Format](http://www.w3.org/TR/WD-logfile.html).
Logging is best done a step in the [PipelineMod.after].  See [util::FileLogger]
