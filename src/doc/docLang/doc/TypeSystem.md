...
state - although they can contain [abstract fields](Fields#abstract-fields) to
...
features too:
  - the `->` [dynamic invoke](Methods#dynamic-invoke) operator lets
    you call any method with runtime checking
  - The compiler will [implicitly cast](#implicit-casts) in most cases for you
  - Type inference is supported for [local variables](Statements#local-variables),
...
and a special [type signature](#list-type-signatures) syntax.
...
and a special [type signature](#map-type-signatures) syntax.
...
generics:
  ...
  - [Map](#map-type-signatures)
  - [Func](#func-type-signature)
...
by the [is/isnot/as](Expressions#type-checking) operators and
...
see [Functions chapter](Functions#type-compatibility) for more
