...
messages on background threads controlled by an [ActorPool](#actor-pools).

# Actors
The [Actor](concurrent::Actor) class is used to define new actors.  All actors
are constructed within an [ActorPool](#actor-pools) which defines how the
actor is executed.
...
# Actor Locals
Actors are [const classes](Classes#const-classes) which means they must be
...
flow control to prevent unbounded queues from growing forever:
  ...
  - Use [coalescing queues](#coalescing-messages) to merge pending messages
