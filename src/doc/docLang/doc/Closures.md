...
syntax as [function types](Functions#function-signatures).  The body of the
...
[call](Functions#calling-functions) `f` like any other function.
...
But remember that we don't have to [use all](Functions#arity-compatibility)
...
It-blocks are a special form of closures with the following differences:
  ...
  - Use [type inference](#closure-type-inference) based on their context
  ...
  - It-blocks are given compile time permission to set const fields
    ...
    constructor (see [const fields](Fields#const-fields))
...
the [return type](Methods#this-returns) of instance methods.  There is
...
With blocks are commonly used with the [comma add operator](Serialization#collection-1)
