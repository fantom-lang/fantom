...
calling the setter method (see [below](#virtual-fields) for an exception to
this rule).

# Calculated Fields
In most cases, the compiler automatically declares storage memory for
your field.  However if the compiler detects that a field's storage is
never accessed then no memory is allocated.  We call these *calculated fields*.
Calculated fields must declare both an explicit getter and setter which does
not use the `&` operator to access the field.  Note that assigning an initial
value using `:=` implicitly uses the storage operator and will allocate storage.
...
field can [override](#overriding-a-method) a virtual method with no parameters.
...
[static initializers](Methods#static-initializers).  Static fields must be
...
hide the setter.  This can be done by narrowing the field's
...
meet one of the following criteria:
  ...
  - [calcualted](#calculated-fields) fields don't need to be set
