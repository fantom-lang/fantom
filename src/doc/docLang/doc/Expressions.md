...
Like the [ternary](#ternary-operator) operator the elvis operator may use
...
[dynamic invoke operator](Methods#dynamic-invoke).
...
if you want to serialize it).  Simple expressions are a subset of
[construction calls](Methods#construction-calls).
...
of how [complex](Serialization#complex) types are serialized.  An example
it-block expression:
...
Fantom has a couple other operators which will be discussed later:
  ...
  - [Call](Functions#calling-functions) operator `()` is used to invoke a function variable.

  - [Dynamic invoke](Methods#dynamic-invoke) operator `->` is used to call
    a method without compile time type checking.

  - [Field storage](Fields#accessor-methods) operator `&` is used to access a field's
