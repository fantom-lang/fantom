...
Abstract classes may or may not contain [abstract methods](Methods#abstract-methods).
...
const classes:
 - A const class can contain only [const fields](Fields#const-fields)
   or fields with no storage:
     a. [abstract fields](Fields#abstract-fields)
     b. [native fields](Natives#native-fields) (native code must ensure thread safety)
     c. [calculated fields](Fields#calculated-fields)
 - A const class cannot contain any [once methods](Methods#once-methods)
...
# Instances
Fantom uses a [pure object oriented](TypeSystem#pure-object-oriented) type system.
