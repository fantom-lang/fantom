...
"
        ...
        private void blah(string[] args)
        {
            string[] installerArgs = new string[0];
            if (args.Length > 0)
            {
                installerArgs = new string[args.Length];
                installerArgs[0] = "/LogToConsole=true";
                Array.Copy(args, 1, installerArgs, 1, args.Length - 1);
                foreach (string arg in installerArgs) Console.WriteLine(arg);
            }
            IDictionary state = new Hashtable();
            string exe = System.Reflection.Assembly.GetExecutingAssembly().Location;
            AssemblyInstaller installer = new AssemblyInstaller(exe, installerArgs);
            installer.UseNewContext = true;
            ...
            if (args.Length > 0 && args[0].ToUpper() == "INSTALL")
            {
                ...
                /*
                FanSvcInstaller installer = new FanSvcInstaller();
            ...
            {
                ServiceBase.Run(new FanService(args));
            }
        }
    }
}
