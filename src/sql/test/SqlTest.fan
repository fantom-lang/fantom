...
//////////////////////////////////////////////////////////////////////////
  ...
  Void batchUpdate()
  {
    ...
    verifyEq(res.updateCounts, Int[,].fill(1, params.size))

    // The keys will all be null, because nothing was autogenerated
    verifyEq(res.keys, Obj?[,].fill(null, params.size))
  ...
  Void batchInsert()
  {
    ...
    if (dbType == DbType.postgres)
      verifyEq(res.keys, Obj?[1,2,3,4])
    else
      verifyEq(res.keys, Obj?[null,null,null,null])
  ...
  Void batchExecutorUpdate()
  {
    ...
    // The result will be an array filled with '1',
    // indicating that each row was updated.
    verifyEq(res.updateCounts, Int?[,].fill(1, ids.size))
