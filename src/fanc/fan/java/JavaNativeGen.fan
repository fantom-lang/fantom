...
//////////////////////////////////////////////////////////////////////////
  ...
  Void copyJarDistEnvJava(File from, File to)
  {
    // add synthetic code
    lines := from.readAllLines
    ...
    to.out.print(s.toStr).close
  }

  private Void jarDistFindAllPodNames(StrBuf s)
  {
    s.add("  /** Auto-generated by fanc */\n")
    s.add("  public List<String> findAllPodNames()\n")
    s.add("  {\n")
    s.add("    List acc = new List(Sys.StrType);\n")
    cmd.pods.each |p|
    {
      s.add("    acc.add($p.name.toCode);\n")
    }
    s.add("    return acc;\n")
    s.add("  }\n")
  }
  ...
  Void copyNativeFiles(PodDef pod, File fromDir, File toDir)
  {
    ...
    fromDir.list.each |from|
    {
      ...
      if (pod.name == "sys")
      {
        ...
        if (from.name == "JarDistEnv.java") return copyJarDistEnvJava(from, to)
      }
