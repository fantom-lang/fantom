...
const class Jwt
{
  ...
  ** Algorithm header
  **
  ** When encoding this value will take precedent if the alg
  ** parameter is also set in the JOSE header
  const Str? alg
  ...
  **
  ** When encoded/decoded, if value is a Str it will converted to a Str[] of size 1
  const Obj? aud
  ...
  ** Decode an unsigned JWT[`Jwt`] from an encoded Str
  **
  ...
  **
  ...
  private static new doDecode(Str encoded, Key? key, Duration clockDrift := 60sec)
  {
    ...
    try
    {
      ...
      if (!header.containsKey(JwtConst.AlgorithmHeader)) throw Err("JWT missing Algorithm (${JwtConst.AlgorithmHeader})")
    ...
    if (!signature.bytesEqual(Buf.fromBase64("")) || jwsAlg != JwsAlgorithm.none || key != null)
    {
      ...
      { 
        if (jwsAlg.keyType != key.algorithm) 
          throw Err("JWT (alg) parameter \"${jwsAlg.toStr}\" is not compatible with Key algorithm \"${key.algorithm}\"")
      ...
      else if (key is MacKey) 
      {
        if(key.algorithm != "Hmac" + jwsAlg.digest) 
          throw Err("JWS (alg) parameter \"${jwsAlg.toStr}\" is not compatible with Key algorithm \"${key.algorithm}\"")
  ...
  ** Provide a Key (PrivKey or MacKey) to sign and return the base64 encoded JWT[`Jwt`]
  **
  ** Null key will return an unsigned base64 encoded JWT
  **
  ** The (alg) parameter must be set to a supported JWS algorithm
  **
  ** The (alg) parameter may be set directly in the headers map
  **
  ** The following JWS Algorithms are supported:
  **
  **   -   HS256 - HMAC using SHA-256
  ...
  **             }.encode(priv)
  **
  **   jwtStr2 := Crypto.cur.encodeJwt(["alg": "RS256"], ["myClaim": "ClaimValue", "exp": DateTime.nowUtc + 10min, "iss": "https://fantom.accounts.dev"], priv)
  **
  Str encode(Key? key)  
  {
    headerParams := addHeaderParameters
    claimsSet := addRegisteredClaims
    
    if (key == null && headerParams[JwtConst.AlgorithmHeader] != "none") 
      throw Err("Header (${JwtConst.AlgorithmHeader}) parameter must be \"none\" if key is null")

    encodedHeader := writeJsonToStr(headerParams).toBuf.toBase64Uri
  ...
  **
  ...
  private Str:Obj addHeaderParameters()
  { 
    params := [:].addAll(header)
    if (kid != null) params[JwtConst.KeyIdHeader] = kid
    params[JwtConst.AlgorithmHeader] = getAlgStr
    return params 
  }

  private Str:Obj addRegisteredClaims()
  {
    ...
    if (jti != null) claimsSet[JwtConst.JwtIdClaim] = Uuid.make.toStr
    
    if (exp != null) claimsSet[JwtConst.ExpirationClaim] = toNumericDate(exp)
    else if (claims[JwtConst.ExpirationClaim] != null) claimsSet[JwtConst.ExpirationClaim] = toNumericDate(claims[JwtConst.ExpirationClaim])

    if (nbf != null) claimsSet[JwtConst.NotBeforeClaim] = toNumericDate(nbf)
    else if (claims[JwtConst.NotBeforeClaim] != null) claimsSet[JwtConst.NotBeforeClaim] = toNumericDate(claims[JwtConst.NotBeforeClaim])

    if (iat != null) claimsSet[JwtConst.IssuedAtClaim] = toNumericDate(iat)
    else if (claims[JwtConst.IssuedAtClaim] != null) claimsSet[JwtConst.IssuedAtClaim] = toNumericDate(claims[JwtConst.IssuedAtClaim])
  ...
  private Str getAlgStr()
  {
    alg != null ? JwsAlgorithm.fromAlg(alg).toStr :
                    JwsAlgorithm.fromParameters(header).toStr
  }

  private JwsAlgorithm getAlg()
  {
    alg != null ? JwsAlgorithm.fromAlg(alg) :
                    JwsAlgorithm.fromParameters(header)
  }
...
enum class JwsAlgorithm
{
  ...
  {
    JwsAlgorithm.fromStr(name.lower)
  }
