...
const class JCsr : Csr
{
  ...
  new make(KeyPair keys, Str subjectDn, Str:Obj opts)
  {
    ...
    {
      sanList := opts["subjectAltNames"] as List
      if (sanList != null && !sanList.isEmpty)
      {
        builder := AsnColl.builder
        SubjectAltNameList := SubjectAltName[,]
        sanList.each |name|
        {
          jSan := JSubjectAltName.fromValue(name)
          SubjectAltNames.encodeName(jSan, builder)
          SubjectAltNameList.add(jSan)
        }
        this.san = builder.toSeq
        this.subjectAltNames = SubjectAltNameList
      }
    }
  }
...
//////////////////////////////////////////////////////////////////////////
  ...
  override const SubjectAltName[] subjectAltNames
...
//////////////////////////////////////////////////////////////////////////
  ...
  ** Decode Subject Alternative Names from CSR attributes (RFC 2986)
  private static SubjectAltName[] decodeSansFromReqInfo(AsnSeq reqInfo)
  {
    try
    {
