...
//////////////////////////////////////////////////////////////////////////
  ...
  Void testReflect()
  {
    ...
    verifyEq([,].type.base,      List#)
    verifyEq([,].type.base.base, Obj#)
    verifyEq([,].type.pod.name,  "sys")
    verifyEq([,].type.name,      "List")
    verifyEq([,].type.qname,     "sys::List")
    verifyEq([,].type.signature, "sys::Obj[]")
    verifyEq([,].type.toStr,     "sys::Obj[]")
    verifyEq([,].type.method("isEmpty").returns,  Bool#)
    verifyEq([,].type.method("get").returns,      Obj#)
    verifyEq([,].type.method("add").returns,      Obj[]#)
    verifyEq([,].type.method("add").params[0].of, Obj#)
    verifyEq([,].type.method("each").params[0].of, |Obj a, Int i->Void|#)
    verifyNotEq([,].type.method("each").params[0].of, |Str a, Int i->Void|#)

    verifyEq([7].type.base,      List#)
    verifyEq([7].type.base.base, Obj#)
    verifyEq([7].type.pod.name,  "sys")
    verifyEq([7].type.name,      "List")
    verifyEq([7].type.qname,     "sys::List")
    verifyEq([7].type.signature, "sys::Int[]")
    verifyEq([7].type.toStr,     "sys::Int[]")
    verifyEq([7].type.method("isEmpty").returns,  Bool#)
    verifyEq([7].type.method("get").returns,      Int#)
    verifyEq([7].type.method("add").returns,      Int[]#)
    verifyEq([7].type.method("add").params[0].of, Int#)
    verifyEq([7].type.method("each").params[0].of, |Int a, Int i->Void|#)
    verifyNotEq([7].type.method("each").params[0].of, |Obj a, Int i->Void|#)

    verifyEq([[8ms]].type.base,      List#)
    verifyEq([[8ms]].type.base.base, Obj#)
    verifyEq([[8ms]].type.pod.name,  "sys")
    verifyEq([[8ms]].type.name,      "List")
    verifyEq([[8ms]].type.qname,     "sys::List")
    verifyEq([[8ms]].type.signature, "sys::Duration[][]")
    verifyEq([[8ms]].type.toStr,     "sys::Duration[][]")
    verifyEq([[8ms]].type.method("isEmpty").returns,   Bool#)
    verifyEq([[8ms]].type.method("get").returns,       Duration[]#)
    verifyEq([[8ms]].type.method("add").returns,       Duration[][]#)
    verifyEq([[8ms]].type.method("add").params[0].of,  Duration[]#)
    verifyEq([8ms].type.method("insert").params[1].of, Duration#)
    verifyEq([8ms].type.method("removeAt").returns,    Duration#)
    verifyEq([8ms].type.method("each").params[0].of, |Duration a, Int i->Void|#)
    verifyEq([8ms].type.method("map").returns, List#)
    verifyNotEq([8ms].type.method("map").returns, Duration[]#)
    verifyNotEq([8ms].type.method("each").params[0].of, |Obj a, Int i->Void|#)
  }
