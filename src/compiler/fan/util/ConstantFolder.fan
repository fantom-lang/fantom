...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  ...
  {
    if (val == null)     return LiteralExpr(orig.location, ExprId.nullLiteral, ns.objType.toNullable, null)
    if (val is Int)      return LiteralExpr(orig.location, ExprId.intLiteral, ns.intType, val)
    if (val is Float)    return LiteralExpr(orig.location, ExprId.floatLiteral, ns.floatType, val)
    if (val is Decimal)  return LiteralExpr(orig.location, ExprId.decimalLiteral, ns.decimalType, val)
    if (val is Str)      return LiteralExpr(orig.location, ExprId.strLiteral, ns.strType, val)
    if (val is Duration) return LiteralExpr(orig.location, ExprId.durationLiteral, ns.durationType, val)
    return orig
  }
